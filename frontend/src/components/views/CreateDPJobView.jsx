...
export function CreateDPJobView({ datasetId, onBackClick, onComplete }) {
    ...
    useEffect(() => {
        ...
        fetchPreview();
    }, [debouncedEpsilon, previewColumn, previewMetric, selectedDatasetId]);
    ...
    const handleSubmit = async () => {
        ...
        const payload = {
            ...
            mechanism: mechanism,
            delta: mechanism === 'gaussian' ? parseFloat(delta) : null,
        };
                        ...
                        <h3 style={{ margin: '0 0 16px' }}>Step 2: Configure DP Parameters</h3>
                         <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                            ...
                            <div style={{ display: 'grid', gap: '16px', alignContent: 'start' }}>
                                <div>
                                    <label style={{display:'block', marginBottom:'8px', color:'var(--muted)'}}>Noise Addition Mechanism</label>
                                    <div style={{display:'flex', gap:'10px'}}>
                                        <label>
                                            <input type="radio" value="laplace" checked={mechanism === 'laplace'} onChange={() => setMechanism('laplace')} />
                                            Laplace
                                        </label>
                                        <label>
                                            <input type="radio" value="gaussian" checked={mechanism === 'gaussian'} onChange={() => setMechanism('gaussian')} />
                                            Gaussian
                                        </label>
                                        <label>
                                            <input type="radio" value="exponential" checked={mechanism === 'exponential'} onChange={() => setMechanism('exponential')} />
                                            Exponential
                                        </label>
                                    </div>
                                </div>
                                ...
                                {mechanism === 'gaussian' && (
                                    <div>
                                        <label htmlFor="delta" style={{display:'block', marginBottom:'8px', color:'var(--muted)'}}>Delta (δ)</label>
                                        <input id="delta" type="number" value={delta} onChange={e => setDelta(e.target.value)} className="form-input" style={{maxWidth:'150px'}}/>
                                    </div>
                                )}
                                <div>

...
import { AirbyteApplicationConnector } from '../connectors/AirbyteApplicationConnector';
...
export function ConnectDataSource({ onBackClick, onComplete }) {
    ...
    const renderConnector = () => {
        switch(connectorType) {
            ...
            case 'airbyte_app': return <AirbyteApplicationConnector onBack={() => setConnectorType(null)} onComplete={onComplete} />;            
            default: return null;
        }
    }
        ...
        <section style={{background:'var(--panel)',border:'1px solid var(--border)',borderRadius:'var(--radius)',padding:'16px',boxShadow:'var(--shadow)'}}>
            ...
            {connectorType ? renderConnector() : (
                <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:'16px'}}>
                    ...
                    <div className="card-hover" onClick={() => setConnectorType('airbyte_app')} style={{padding:'16px', background:'var(--panel-2)', borderRadius:'var(--radius-sm)', cursor:'pointer'}}>
                        <h3 style={{margin:0}}>Airbyte Cloud App</h3>
                        <p style={{color:'var(--muted)', fontSize:'14px'}}>Connect via a secure Airbyte Application.</p>
                    </div>
                </div>
            )}
        </section>

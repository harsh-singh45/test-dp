...
import { DPJobRegistry } from './components/views/DPJobRegistry';
import { BudgetManager } from './components/views/BudgetManager';
import { PolicyManager } from './components/views/PolicyManager';
import { DPJobDetailView } from './components/views/DPJobDetailView';

export default function App() {
    // view can be: 'dashboard', 'datasets', 'dataset_detail', 'register', 'import_schema', 'create_job', 'jobs', 'job_detail', 'budget', 'policy'
    ...
    const handleCreateJobClick = (datasetId = null) => {
    ...
    const renderView = () => {
        switch (view) {
            case 'dashboard':
                return <DashboardView onCreateJobClick={() => setView('create_job')} onJobClick={handleJobClick} />;
            case 'register':
            ...
            case 'dataset_detail':
                return <DatasetDetailView datasetId={currentDatasetId} onBackClick={() => setView('datasets')} onCreateJobClick={handleCreateJobClick} />;
            case 'create_job':
                return <CreateDPJobView datasetId={currentDatasetId} onBackClick={() => setView('datasets')} onComplete={() => setView('jobs')} />;
            case 'jobs':
                 return <DPJobRegistry onJobClick={handleJobClick} />;
            case 'job_detail':
                return <DPJobDetailView jobId={currentJobId} onBackClick={() => setView('jobs')} />;
            case 'budget':
    ...
    // Determine which main tab should be active for the Topbar
    const getActiveTab = () => {
        if (['datasets', 'dataset_detail', 'register', 'import_schema'].includes(view)) {
            return 'datasets';
        }
        if (['jobs', 'job_detail', 'create_job'].includes(view)) {
            return 'jobs';
        }
        return view; // 'dashboard', 'budget', 'policy'
    };

    return (
        <>
            <GlobalStyles />
            <Topbar activeView={getActiveTab()} setActiveView={setView} />

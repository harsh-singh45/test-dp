...
// dpApiService is no longer needed for this component
// import { dpApiService } from '../../services/dpApiService';

// --- FIX START ---
// The backend doesn't have an endpoint for mechanisms.
// Since they are fixed, we can define them directly here.
const staticMechanisms = [
  {
    id: 'laplace',
    name: 'Laplace Mechanism',
    type: 'additive_noise',
    description: 'Adds precisely calibrated noise from a Laplace distribution to the true result. The amount of noise is directly proportional to the query\'s sensitivity and inversely proportional to epsilon.',
    performance_rating: 4.5,
    parameters: ['epsilon', 'sensitivity'],
    use_cases: ['COUNT', 'SUM', 'MEAN'],
    privacy_guarantee: 'ε-DP (pure Epsilon-Differential Privacy)',
  },
  {
    id: 'gaussian',
    name: 'Gaussian Mechanism',
    type: 'additive_noise',
    description: 'Adds noise from a Gaussian (Normal) distribution. It provides (ε, δ)-differential privacy, where delta is a small probability that the privacy guarantee might not hold, often allowing for less noise.',
    performance_rating: 4.2,
    parameters: ['epsilon', 'delta', 'sensitivity'],
    use_cases: ['SUM', 'MEAN', 'VARIANCE'],
    privacy_guarantee: '(ε, δ)-DP (Epsilon-Delta-Differential Privacy)',
  }
];
// --- FIX END ---

const MechanismsContent = () => {
  // --- FIX: State is now initialized with our static data ---
  const [mechanisms, setMechanisms] = useState(staticMechanisms);
  // Loading and error are no longer needed, but we keep them to avoid breaking the UI logic
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [selectedMechanism, setSelectedMechanism] = useState(null);

  // --- FIX: The useEffect hook for fetching data is no longer needed ---
  /*
      ...
      {loading ? (
        ...
        <div className="row">
          {mechanisms.map((mechanism, index) => (
            <div key={mechanism.id} className="col-lg-4 mb-4">

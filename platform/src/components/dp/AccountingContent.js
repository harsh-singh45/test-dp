...
const AccountingContent = () => {
  ...
  const [datasetToReset, setDatasetToReset] = useState(''); // **FIXED: Declared the missing state**
  ...
  const handleOpenResetModal = (dataset) => {
    // If a dataset is passed (from the table row), pre-select it.
    // Otherwise, clear it so the modal shows the dropdown.
    setSelectedDataset(dataset || null);
    setShowResetModal(true);
  };
  
  const handleOpenAllocateModal = () => {
    // When opening, set the form values to 0
    setEpsilonToAdd(0);
    setDeltaToAdd(0);
  ...
  const handleOpenAlertModal = async () => {
      // Just open the modal. The useEffect below will fetch the alerts.
  ...
  const handleResetBudget = async (datasetToActOn) => {
      ...
      try {
        ...
        setSelectedDataset(null); // Clear selection after action
        fetchBudgetData();
      } catch (err) {
  ...
  const handleCreateAlert = async () => {
      const dataset = budgetData.datasets.find(d => d.name === datasetForAlerts);
      if (!selectedDataset || !alertEmail) return;
      try {
          const alertData = { threshold: parseFloat(alertThreshold), email: alertEmail };
          await dpApiService.createAlert(dataset.id, alertData);
          const fetchedAlerts = await dpApiService.getAlerts(dataset.id);
          setAlerts(fetchedAlerts);
          setAlertEmail('');
      } catch (err) {
  ...
  return (
    <div>
      ...
      {/* Set Alert Modal */}
      {showAlertModal && selectedDataset && (
         <div className="modal show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
            <div className="modal-dialog modal-dialog-centered">
                <div className="modal-content">
                    <div className="modal-header">
